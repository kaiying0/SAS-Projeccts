...
LIBNAME PUF "C:\Users\Kaiying\Documents\My SAS Files\NISTEEN2018";
OPTIONS FMTSEARCH = (PUF WORK LIBRARY);

%LET MYPATH = C:\Users\Kaiying\Documents\My SAS Files\NISTEEN2018\RESULTS1;
...
PROC FREQ DATA = PROJECT;
	WHERE PDAT2 = 1; /*** USING ONLY ADEQUATE PROVIDER DATA ***/
	WEIGHT PROVWT_C; /*** TO INCLUDE WEIGHTS FOR WEIGHTED RESULTS ***/
	TABLES (NEWAGE TEENAGEGP SEX MYRACEETH EDUC1 MARITAL2 AGEGRP_M_I MYINCTOPOV INS_STAT2_I MYCKUP1112 CEN_REG MYFACILITY)*P_UTDHPV_15INT / CHISQ;
	TITLE1 '2018 NIS-TEEN PUBLIC-USE FILE';
	TITLE2 'CHI-SQUARE TEST RESULTS FOR WEIGHTED VACCINATION COVERAGES BY SELECTED VARIABLES';
...
PROC SGPLOT DATA=PROJECT;
	WHERE PDAT2 = 1; /*** USING ONLY ADEQUATE PROVIDER DATA ***/
    TITLE "Plotting Weighted Histogram Using Mother's Age (in Years) at Interview in NIS-Teen 2018 PUF";
    HISTOGRAM AGEGRP_M_I / BINSTART = 1 BINWIDTH = 1 NBINS = 5 WEIGHT = PROVWT_C;
    DENSITY AGEGRP_M_I;
RUN;

PROC SGPLOT DATA=PROJECT;
	WHERE PDAT2 = 1; /*** USING ONLY ADEQUATE PROVIDER DATA ***/
    TITLE "Plotting Weighted Histogram Using Income to Poverty Ratio at Interview in NIS-Teen 2018 PUF";
    HISTOGRAM INCPORAR_I / BINSTART = 1 BINWIDTH = 1 NBINS = 5 WEIGHT = PROVWT_C;
    DENSITY INCPORAR_I;
RUN;
...
PROC SGPLOT DATA=PROJECT;
	WHERE PDAT2 = 1; /*** USING ONLY ADEQUATE PROVIDER DATA ***/
    TITLE "Weighted Vertical Bar Chart Using Race/Ethnicity of Teen in NIS-Teen 2018 PUF";
    VBAR MYRACEETH / WEIGHT = PROVWT_C STAT = PERCENT;
    FORMAT MYRACEETH FMTRACE.;
RUN;

PROC SGPLOT DATA=PROJECT;
	WHERE PDAT2 = 1; /*** USING ONLY ADEQUATE PROVIDER DATA ***/
    TITLE "Weighted Vertical Bar Chart Using Education Level of Mother in NIS-Teen 2018 PUF";
    VBAR EDUC1 / WEIGHT = PROVWT_C STAT = PERCENT;
    FORMAT EDUC1 FMTMEDU.;
RUN;

PROC FORMAT;
	VALUE PLOTMARRY 1 = 'Married'
					2 = 'Not Married';
RUN;
PROC SGPLOT DATA=PROJECT;
	WHERE PDAT2 = 1; /*** USING ONLY ADEQUATE PROVIDER DATA ***/
    TITLE "Weighted Vertical Bar Chart Using Mother's Marital Status in NIS-Teen 2018 PUF";
    VBAR MARITAL2 / WEIGHT = PROVWT_C STAT = PERCENT;
	FORMAT MARITAL2 PLOTMARRY.;
RUN;
...
PROC GCHART DATA=PROJECT;
   TITLE1 "Unweighted Pie-Chart";
   TITLE2 "Using the Medical Insurance Status of the Adolescent in NIS-Teen 2018 PUF";
   WHERE PDAT2 = 1  AND PROVWT_C NE .; /*** USING ONLY ADEQUATE PROVIDER DATA ***/
   PIE INS_STAT2_I / SUMVAR = PROVWT_C TYPE=SUM PERCENT = ARROW NOHEADING LEGEND DISCRETE;
   FORMAT INS_STAT2_I FMTINS.;
RUN;
QUIT;
PROC GCHART DATA=PROJECT;
   TITLE1 "Unweighted Pie-Chart";
   TITLE2 "Using the Status of Well Child Visit at Age 11-12 years in NIS-Teen 2018 PUF";
   WHERE PDAT2 = 1  AND PROVWT_C NE .; /*** USING ONLY ADEQUATE PROVIDER DATA ***/
   PIE CKUP_11_12 / SUMVAR = PROVWT_C TYPE=SUM PERCENT = ARROW NOHEADING LEGEND DISCRETE;
   FORMAT CKUP_11_12 FMTCKUP.;
RUN;
QUIT;
PROC GCHART DATA=PROJECT;
   TITLE1 "Unweighted Pie-Chart";
   TITLE2 "Using the Metropolitan Statistical Area of the Adolescent in NIS-Teen 2018 PUF";
   WHERE PDAT2 = 1  AND PROVWT_C NE .; /*** USING ONLY ADEQUATE PROVIDER DATA ***/
...
PROC GCHART DATA=PROJECT;
   TITLE1 "Unweighted Pie-Chart";
   TITLE2 "Using the Facility Types for Teen's Providers in NIS-Teen 2018 PUF";
   WHERE PDAT2 = 1  AND PROVWT_C NE .; /*** USING ONLY ADEQUATE PROVIDER DATA ***/
   PIE FACILITY / SUMVAR = PROVWT_C TYPE=SUM PERCENT = ARROW NOHEADING LEGEND DISCRETE;
   FORMAT FACILITY FMTFACTY.;
RUN;
QUIT;
...
%MYESTIMATE(NEWAGE, P_UTDHPV_15INT, MYAGE., 1, "Age (years) at Interview");
%MYESTIMATE(TEENAGEGP, P_UTDHPV_15INT, FMTAGRP., 2, "Age Group (in years) at Interview");
%MYESTIMATE(SEX, P_UTDHPV_15INT, FMTSEX., 3, "Sex of Adolescent at Interview");
%MYESTIMATE(MYRACEETH, P_UTDHPV_15INT, FMTRACE., 4, "Sex of Adolescent at Interview");
%MYESTIMATE(EDUC1, P_UTDHPV_15INT, FMTMEDU., 5, "Sex of Adolescent at Interview");
%MYESTIMATE(MARITAL2, P_UTDHPV_15INT, FMTMARRY., 6, "Sex of Adolescent at Interview");
%MYESTIMATE(AGEGRP_M_I, P_UTDHPV_15INT, FMTMAGRP., 7, "Sex of Adolescent at Interview");
%MYESTIMATE(MYINCTOPOV, P_UTDHPV_15INT, FMTINCPR., 8, "Sex of Adolescent at Interview");
%MYESTIMATE(INS_STAT2_I, P_UTDHPV_15INT, FMTINS., 9, "Sex of Adolescent at Interview");
%MYESTIMATE(MYCKUP1112, P_UTDHPV_15INT, FMTCKUP., 10, "Sex of Adolescent at Interview");
%MYESTIMATE(CEN_REG, P_UTDHPV_15INT, FMTCENREG., 11, "Sex of Adolescent at Interview");
%MYESTIMATE(MYFACILITY, P_UTDHPV_15INT, FMTFACTY., 12, "Sex of Adolescent at Interview");
%MYESTIMATE(STATE, P_UTDHPV_15INT, FMSTATE., 13, "Sex of Adolescent at Interview");
...
PROC SURVEYFREQ DATA = PROJECT;
    ...
    TABLES  PDAT2*(P_UTDHPV_15INT NEWAGE TEENAGEGP SEX MYRACEETH EDUC1 MARITAL2 AGEGRP_M_I MYINCTOPOV INS_STAT2_I MYCKUP1112 CEN_REG MYFACILITY STATE)/ CL (TYPE=CP) NOWT NOSTD;
RUN;
...
DATA FINALSTABLE1V1;
	...
	SET FINALSTABLE1(KEEP = AGEGRP_M_I CEN_REG EDUC1 FREQUENCY INS_STAT2_I LOWERCL MARITAL2 MYCKUP1112 MYFACILITY MYINCTOPOV 
					 MYRACEETH NEWAGE TEENAGEGP STATE PDAT2 P_UTDHPV_15INT PERCENT SEX TABLE UPPERCL _SKIPLINE);
...
DATA FINALSTABLE1V2;
	...
	/*** MYVARIABLE = THE LABEL FOR EACH VALUE OF VARIABLE; MYVARCOUNT = THE VARIABLE NUMBER IN THE TABLE; MYVARNUM = THE VALUE OF EACH VARIABLE ***/
	IF P_UTDHPV_15INT NE . THEN DO;
		MYVARIABLE = PUT(P_UTDHPV_15INT, FMTUTD.);
		MYVARCOUNT = 0;
		MYVARNUM = P_UTDHPV_15INT;
	END;
    IF NEWAGE NE . THEN DO;
		MYVARIABLE = PUT(NEWAGE, MYAGE.);
		MYVARCOUNT = 1;
...
/**** 				BELOW ALSO CREATES TABLE 4						 ***/

PROC EXPORT DATA = FINALSTABLE1V2 OUTFILE ="&MYPATH.\TABLE1\SURVEYTABLE1.xlsx" DBMS=XLSX REPLACE;
RUN;

/***********************************************************************/
/****             UNADJUSTED   MODEL 1  --- TABLE 5 COLUMN 1        ****/
/***********************************************************************/
...
%MYESTIMATES(P_UTDHPV_15INT, NEWAGE, MYAGE., 1, "AGE IN YEARS OF SELECTED TEEN");
%MYESTIMATES(P_UTDHPV_15INT, TEENAGEGP, FMTAGRP., 2, "AGE IN YEARS OF TEEN GROUP");
%MYESTIMATES(P_UTDHPV_15INT, SEX, FMTSEX., 3, "SEX OF TEEN");
%MYESTIMATES(P_UTDHPV_15INT, MYRACEETH, FMTRACE., 4, "RACE/ETHNICITY OF TEEN WITH MULTIRACE CATEGORY (RECODE)");
%MYESTIMATES(P_UTDHPV_15INT, EDUC1, FMTMEDU., 5, "EDUCATION LEVEL OF MOTHER WITH 4 CATEGORIES (RECODE)");
%MYESTIMATES(P_UTDHPV_15INT, MARITAL2, FMTMARRY., 6, "MARITAL STATUS OF MOTHER (RECODE)");
%MYESTIMATES(P_UTDHPV_15INT, AGEGRP_M_I, FMTMAGRP., 7, "MOTHER'S AGE CATEGORIES (RECODE)");
%MYESTIMATES(P_UTDHPV_15INT, MYINCTOPOV, FMTINCPR., 8, "(INCOME TO POVERTY RATIO: IMPUTED (RECODE)");
%MYESTIMATES(P_UTDHPV_15INT, INS_STAT2_I, FMTINS., 9, "INSURANCE STATUS (PRIVATE ONLY/ANY MEDICAID/OTHER INSURANCE/UNINSURED): IMPUTED");
%MYESTIMATES(P_UTDHPV_15INT, MYCKUP1112, FMTCKUP., 10, "DID TEEN HAVE AN 11-12 YEAR OLD WELL-CHILD EXAM OR CHECK-UP?");
%MYESTIMATES(P_UTDHPV_15INT, CEN_REG, FMTCENREG., 11, "CENSUS REGION BASED ON TRUE STATE OF RESIDENCE");
%MYESTIMATES(P_UTDHPV_15INT, MYFACILITY, FMTFACTY., 12, "FACILITY TYPES FOR TEEN'S PROVIDERS");

DATA PUF.FINALTABLE2;
	SET FINALTABLE2;
RUN;	
PROC EXPORT DATA = FINALTABLE2 OUTFILE= "&MYPATH.\TABLE5\TABLE5COLUMN1.xlsx"
	DBMS=XLSX REPLACE;
RUN;
...
PROC SURVEYLOGISTIC DATA = PROJECT;
    ...
    MODEL  P_UTDHPV_15INT(EVENT = "1") = &VAR1.;
...
PROC EXPORT DATA = ODDSRATIOSTABLE3 OUTFILE ="&MYPATH.\TABLE5\ODDSRATIOSTABLE5.xlsx" DBMS=XLSX REPLACE;
RUN;
...
PROC EXPORT DATA = MYUNADJPARATTEST OUTFILE ="&MYPATH.\TABLE5\MYUNADJPARATTESTTABLE5.xlsx" DBMS=XLSX REPLACE;
RUN;
...
PROC EXPORT DATA = MYTYPE3TEST OUTFILE ="&MYPATH.\TABLE6\MYTYPE3TESTTABLE6.xlsx" DBMS=XLSX REPLACE;
RUN;
...
PROC SURVEYLOGISTIC DATA = PROJECT;
    ...
    MODEL  P_UTDHPV_15INT(EVENT = "1") = NEWAGE SEX MYRACEETH EDUC1 MARITAL2 AGEGRP_M_I MYINCTOPOV INS_STAT2_I MYCKUP1112 CEN_REG MYFACILITY;
...
PROC EXPORT DATA = MYADJORESTV2 OUTFILE ="&MYPATH.\TABLE5\MULTILOGODDSRATIOTABLE5.xlsx" DBMS=XLSX REPLACE;
RUN;
...
PROC EXPORT DATA = MYADJPARAESTV1 OUTFILE ="&MYPATH.\TABLE3\MULTILOGTTESTTABLE3.xlsx" DBMS=XLSX REPLACE;
RUN;
...
PROC EXPORT DATA = MYADJTYPE3TESTV1 OUTFILE ="&MYPATH.\TABLE6\MULTILOGTYPE3TESTTABLE6.xlsx" DBMS=XLSX REPLACE;
RUN;
...
PROC SURVEYLOGISTIC DATA = PROJECT;
	...
	CLASS NEWAGE(PARAM=REF REF="5") EDUC1(PARAM=REF REF="1") MYCKUP1112(PARAM=REF REF="1") CEN_REG(PARAM=REF REF="1") MYFACILITY(PARAM=REF REF="1");

    MODEL  P_UTDHPV_15INT(EVENT = "1") = NEWAGE EDUC1 MYCKUP1112 CEN_REG MYFACILITY;
...
PROC EXPORT DATA = MYORESTML4V2 OUTFILE ="&MYPATH.\TABLE5\MULTILOGODDSRATIOTABLE5ML4.xlsx" DBMS=XLSX REPLACE;
RUN;
...
PROC EXPORT DATA = MYTYPE3TESTML4V1 OUTFILE ="&MYPATH.\TABLE6\MULTILOGTYPE3TESTTABLE6ML4.xlsx" DBMS=XLSX REPLACE;
RUN;
